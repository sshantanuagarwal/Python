...
public class RotenOrange {
	...
	private static Queue<element> formFreshQueue(int[][] inp2dArray) {
		Queue<element> elemQueue = new LinkedList<element>();
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 5; j++) {
				if (inp2dArray[i][j] == 1) {
	...
	private static int minimumTimeToRotAllOranges(int[][] inp2dArray, Queue<element> formFreshQueue, int minTime) {
		minTime++;
		Queue<element> toBeRotten = new LinkedList<element>();
		Queue<element> stillFresh = new LinkedList<element>();
