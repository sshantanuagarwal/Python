...
int manhattan(int** ini,int** fin)
{
    ...
    if (*(*(fin+k)+l)==*(*(ini+i)+j))
    {
    manhattan=manhattan + (fabs(i-k)+fabs(j-l));}
   }
   }
       }
            }
...
void steepestHillClimb( int** ini, int** fin,int previous,int previous_man)
{
    ...
    int locOpt=previous_man;
    int index=0;
    for(int i=0;i<4;i++)
    {
        if(arr[i]<previous_man)
        {
            locOpt=arr[i];
            index=i+1;
            break;
    ...
    cout<<previous_man<<"previous value"<<endl;
    makeMove(ini,index);
    cout<<"Next state (the one with lesser Manhattan number than previous) :"<<endl;
    printArray(ini);
    if(locOpt==0)
    {
        cout<<"Solved! :)"<<endl;
    ...
    steepestHillClimb(ini,fin,index,locOpt);
}
...
int main()
{
    ...
    man=manhattan(initial,final);
    steepestHillClimb(initial,final,0,man);
    for(int i=0;i<3;i++)
    {
        delete initial[i];
        delete final[i];
